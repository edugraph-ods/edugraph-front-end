...
interface CourseGraphProps {
  ...
  planResult?: { total_cycles: number; cycles: { cycle: number; total_credits: number; courses: string[] }[] } | null;
}
...
const CourseGraph: FC<CourseGraphProps> = ({
  ...
  const renderPlan = (
    result: { total_cycles: number; cycles: { cycle: number; total_credits: number; courses: string[] }[] }
  ) => {
    const nodeMap = new Map(nodes.map((n) => [n.id, n] as const));
    return (
      <div className="h-full flex flex-col">
        <div className="space-y-2">
          <h3 className="text-lg font-medium text-foreground">{t('detail.title')}</h3>
          <p className="text-sm text-muted-foreground">{t('detail.credits')}: {result.total_cycles}</p>
        </div>
        <div className="flex-1 min-h-0 overflow-auto space-y-4">
        {result.cycles.map((cyc) => {
          const items = (cyc.courses || []).map((entry: any) => {
            const courseCode: string = typeof entry === 'string' ? entry : (entry?.code ?? '');
            const n = nodeMap.get(courseCode);
            const c = scheduleCourses.find((sc) => sc.id === courseCode) || courses.find((sc) => sc.id === courseCode);
            const labelFromEntry = typeof entry === 'object' && entry?.name ? entry.name : undefined;
            const creditsFromEntry = typeof entry === 'object' && typeof entry?.credits === 'number' ? entry.credits : undefined;
            return {
              id: courseCode,
              cycle: cyc.cycle,
              label: n?.data.label ?? c?.name ?? labelFromEntry ?? courseCode,
              credits: typeof c?.credits === 'number' ? (c as any).credits : (creditsFromEntry ?? 0),
              prereqs: (n?.data?.prerequisites ?? c?.prerequisites ?? []).filter((p: string) => p && (nodeMap.has(p) || scheduleCourses.some(sc => sc.id === p)))
            };
          });
          return (
            <div key={`plan-cyc-${cyc.cycle}`} className="space-y-2">
              <h4 className="text-md font-medium text-foreground/80">{t('detail.cycle')} {cyc.cycle}</h4>
              {items.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 items-stretch" style={{ gridAutoRows: 'minmax(180px, auto)' }}>
                  {items.map((item, idx) => (
                    <div
                      key={`${item.id}-${idx}`}
                      className={`p-4 rounded-lg border ${
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">{`${item.credits} ${item.credits === 1 ? t('filters.detail.credit') : t('filters.detail.credits')}`}</p>
                        </div>
                        <div className="relative">
                          <select
                            value={getCurrentStatus(item.id)}
                            ...
                            onClick={(e) => e.stopPropagation()}
                            className={`px-3 py-1 rounded-full text-xs font-medium text-center min-w-[110px] cursor-pointer appearance-none ${getStatusColor(getCurrentStatus(item.id))} hover:opacity-90 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-ring`}
                          >
                            <option value="not_taken">{t('filters.detail.selector.faild')}</option>
                            <option value="approved">{t('filters.detail.selector.approved')}</option>
                            <option value="failed">{t('filters.detail.selector.reprobated')}</option>
                      ...
                      {item.prereqs.length > 0 && (
                        <div className="mt-2 pt-2 border-t border-border">
                          <p className="text-xs text-muted-foreground mb-1">{t('filters.required')}</p>
                          <div className="flex flex-wrap gap-1">
                            {item.prereqs.map((prereqId, index) => (
                              <span
                                key={`${prereqId}-${index}`}
                              ...
                              >
                                {nodeMap.get(prereqId)?.data.label ?? scheduleCourses.find((c) => c.id === prereqId)?.name ?? prereqId}
                              </span>
                            ))}
                          </div>
                        </div>
                      ...
                      <div className="mt-3 flex justify-end">
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); setDetailCourseId(item.id); }}
                          className="px-3 py-1.5 text-xs font-medium text-primary border border-input hover:bg-accent rounded-md focus:outline-none focus:ring-2 focus:ring-ring"
                        >
                          {t('filters.detail.button')}
                        </button>
  ...
  return (
    ...
    >
      <div className={`relative ${hasPlan ? 'hidden' : 'flex-1'}`} style={{ minHeight: 0 }}>
        {selectedCourseId ? (
          <ReactFlow
            nodes={nodes}
            edges={edges}
      ...
      <div
        className={`${hasPlan ? '' : 'border-t border-border'} p-4 bg-card shadow-inner flex-1`}
        style={{ minHeight: 0, overflowY: 'auto' }}
      >
        {planResult ? renderPlan(planResult) : renderSchedule()}
      </div>
